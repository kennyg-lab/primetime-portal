...
.rwBtn{margin-left:auto;font-size:10px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#888;background:#F0F0F0;border:1px solid #DDD;border-radius:3px;padding:4px 10px;cursor:pointer;font-family:"Inter",sans-serif}
  ...
  res.send(shell('Edit Report','dash',`
    ...
    <div class="layout">
      ...
      <div class="ed">
        ...
        <div class="sec" id="s3">
          <div class="sh"><span class="sn">03</span><span class="st2">Inspection Findings</span><button class="rwBtn" data-sec="findings" type="button">↻ Rewrite</button></div>
          <div class="fg one"><div class="fld"><label>Findings Narrative</label><textarea id="findTxt" rows="6">${r.findings||''}</textarea></div></div>
        </div>
        <div class="sec" id="s4">
          <div class="sh"><span class="sn">04</span><span class="st2">Cause of Damage</span><button class="rwBtn" data-sec="causeD" type="button">↻ Rewrite</button></div>
          <div class="fg one"><div class="fld"><label>Detailed Cause</label><textarea id="causeD" rows="5">${r.causeD||''}</textarea></div></div>
        </div>
        <div class="sec" id="s5">
          <div class="sh"><span class="sn">05</span><span class="st2">Repair Recommendation</span><button class="rwBtn" data-sec="rec" type="button">↻ Rewrite</button></div>
          <div class="fg one">
        ...
        <div class="sec" id="s6">
          <div class="sh"><span class="sn">06</span><span class="st2">Summary</span><button class="rwBtn" data-sec="summary" type="button">↻ Rewrite</button></div>
...
- NO urgency language`;
  ...
  try {
    ...
    if (sec === 'rec') {
      prompt = `Rewrite the Repair Recommendation section for this insurance inspection report. Produce a fresh alternative wording — different phrasing from before, same facts.

${RULES}

${context}

Return ONLY this JSON: {"rec":"1-2 concise sentences: what is recommended and why","repair":"1 concise sentence: specifically what work must be done"}`;
      parseAsJson = true;
    } else {
      const labels = { findings:'Inspection Findings', causeD:'Cause of Damage', repair:'Repair Detail', summary:'Summary' };
      const lengths = { findings:'2-3 concise sentences', causeD:'2-3 concise sentences', repair:'1 concise sentence', summary:'2-3 concise sentences, do NOT repeat the property address or year built' };
      prompt = `Rewrite the "${labels[sec]}" section for this insurance inspection report. Produce a fresh alternative wording — different phrasing and structure from the existing version, same underlying facts.

${RULES}

${context}

Write ${lengths[sec]}. Return ONLY the rewritten text for this section — no JSON, no labels, no quotes, just the prose.`;
    }
...
app.post('/api/report/:id', requireAuth, (req,res) => {
  const existing = REPORTS[req.params.id] || {id:req.params.id,createdAt:new Date().toISOString(),status:'pending'};
  if (req.body.photos) {
...
app.post('/api/word/:id', requireAuth, async (req,res) => {
  ...
  try {
    ...
    if(secs.length===0){
      ...
      for(const line of r.reportText.split('\n')){
        if(line.trim()) children.push(new Paragraph({children:[new TextRun({text:line.trim(),size:20,color:'333333'})],spacing:{after:120}}));
      }
    }
      ...
      for(let i=0;i<pItems.length;i+=2){
        ...
        for(const p of row){
          try{
            ...
            rc.push(new ImageRun({data:buf,transformation:{width:270,height:185},type:isPng?'png':'jpg'}));
    ...
    children.push(new Paragraph({
      children:[new TextRun({text:'Confidential — Prepared for Insurance Purposes Only',size:14,color:'999999',italics:true})],
      alignment: AlignmentType.RIGHT
    }));
    children.push(new Paragraph({
      children:[new TextRun({text:'Prime Time Electricians  |  ABN 88 151 349 012  |  EC 9142',size:14,color:'999999'})],
      alignment: AlignmentType.RIGHT
    }));
