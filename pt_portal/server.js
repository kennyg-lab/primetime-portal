...
app.post('/api/generate/:id', requireAuth, async (req,res) => {
  const r = REPORTS[req.params.id];
  if (!r) return res.status(404).json({ok:false,error:'Report not found'});
  // Save edits from form — but preserve existing photos (they have base64 data)
  const existingPhotos = r.photos;
  Object.assign(r, req.body);
  // Restore photos with data if the submitted photos lost their base64
  if (req.body.photos) {
    r.photos = req.body.photos.map((p, i) => ({
      data: (p.data && p.data.length > 100) ? p.data : (existingPhotos[i]?.data || null),
      caption: p.caption || existingPhotos[i]?.caption || 'Photo '+(i+1)
    }));
  } else {
    r.photos = existingPhotos;
  }
  saveReport(r);
  const d = r;
...
app.post('/api/report/:id', requireAuth, (req,res) => {
  ...
  // Preserve photo data if submitted photos lost their base64
  if (req.body.photos) {
